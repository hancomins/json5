...
import com.hancomins.json5.container.json5.ParsingState;
...
public class BinaryCSONParser {
	...
	public static JSON5Element parse(byte[] buffer) {
	...
	public static JSON5Element parse(ByteBuffer buffer) {
		BinaryCSONParseIterator binaryCsonParseIterator = new BinaryCSONParseIterator();
		BinaryCSONBufferReader.parse(buffer, binaryCsonParseIterator);
		return binaryCsonParseIterator.release();
	}

	public static JSON5Element parse(byte[] buffer, int offset, int len) {
		BinaryCSONParseIterator binaryCsonParseIterator = new BinaryCSONParseIterator();
	...
	public BaseDataContainer parse(InputStream inputStream, BaseDataContainer rootDataContainer) throws IOException {
		...
		if(footer != CSONFlag.JSON5_FOOTER) {
			throw new JSON5ParseException("Invalid JSON5Type Footer");
		}
	...
	private void upParentContainer() {
		try {
			...
			// todo : 메시지 수정
			throw new JSON5ParseException("End of Container", e);
		}
	}
	...
	private int readCommentCount(int commentFlag) throws IOException {
		switch (commentFlag) {
			...
			default:
				throw new JSON5ParseException("Invalid Comment Type");
		}
	}
		...
		for (int i = 0; i < commentCount; i++) {
			...
			if(arrayComments) {
				...
				try {
					...
					throw new JSON5ParseException("Invalid Comment Index Type");
				}
			} else {
	...
	private int readHeader() throws IOException {
		...
		if(value != CSONFlag.CSON_HEADER) {
			throw new JSON5ParseException("Invalid JSON5Type Header");
		}
		int version = dataInputStream.readShort();
		if(version != CSONFlag.CSON_VERSION) {
			throw new JSON5ParseException("Invalid JSON5Type Version. Current version is " + CSONFlag.CSON_VERSION + " but read version is " + version);
		}
		...
		if(type == CSONFlag.TYPE_ARRAY_LESS_THAN_16) {
			...
			switch (state) {
				...
				case CSONFlag.ARRAY_UINT32:
					...
					break;
				default:
					throw new JSON5ParseException("Invalid String Table Type");
			}
		}
	...
	private int readHeaderComment(int state) throws IOException {
		...
		if (state == CSONFlag.HEADER_COMMENT) {
			...
			throw new JSON5ParseException("Invalid Header Comment Type");
		}
	...
	private void readFooterComment() throws IOException {
		...
		if(state == (byte)CSONFlag.FOOTER_COMMENT) {
			...
			throw new JSON5ParseException("Invalid Footer Comment Type");
		}
	}
		...
		if(hasStringTable) {
			...
			switch (keyType) {
				...
				case CSONFlag.INT32:
					...
					break;
				default:
					throw new JSON5ParseException("Invalid String Table Index Type");
			}
	...
	private int readObjectCount(int state) throws IOException {
		...
		switch (state) {
            ...
            case CSONFlag.ARRAY_UINT32:
				...
				break;
            default:
				throw new JSON5ParseException("Invalid Container length type");
		}
	...
	private Object readNumerable(int state) throws IOException {
		switch (state) {
			...
			default:
				throw new JSON5ParseException("Invalid Integer Type");
		}
	}
	...
	private Number readFloat(int state) throws IOException {
		switch (state) {
			...
			default:
				throw new JSON5ParseException("Invalid Float Type");
		}
	}
	...
	private int readStringLength(int stringType) throws IOException {
		...
		switch (stringType) {
			...
			case CSONFlag.STRING_UINT32:
				...
				break;
			default:
				throw new JSON5ParseException("Invalid String length type");
		}
		return lengthInt32;
	}
	...
	private byte[] readByteBuffer(int state) throws IOException {
		...
		switch (state) {
			...
			case CSONFlag.BYTE_BUFFER_UINT32:
				...
				break;
			default:
				throw new JSON5ParseException("Invalid ByteBuffer length type");
		}
	...
	private Object readStringOrByteBuffer(int state) throws IOException {
		switch (state) {
			...
			default:
				throw new JSON5ParseException("Invalid String or ByteBuffer length type");
		}
	}
	...
	private Object readValue(int state) throws IOException {
		...
		switch (type) {
			...
			default:
				throw new JSON5ParseException("Invalid Value Type");
		}
