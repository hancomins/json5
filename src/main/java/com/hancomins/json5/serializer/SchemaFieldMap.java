...
class SchemaFieldMap extends SchemaField implements ISchemaMapValue {
    ...
    SchemaFieldMap(TypeSchema parentsTypeSchema, Field field, String path) {
        ...
        if(!String.class.isAssignableFrom(keyClass)) {
            throw new JSON5SerializerException("Map field '" + fieldPath + "' is not String key. Please use String key.");
        }
    ...
    public Object newInstance() {
        try {
            ...
            throw new JSON5SerializerException("Map type " + field.getDeclaringClass().getName() + "." + field.getType().getName() + " has no default constructor.", e);
        }
    }
