...
public class ObtainTypeValueInvoker {
            ...
            for(Method method : methods) {
                ...
                if(obtainTypeValue != null) {
                    ...
                    for(String fieldName : fieldNames) {
                        ...
                        if(allFields.stream().filter(field -> field.getAnnotation(JSON5Value.class) != null).noneMatch(field -> field.getName().equals(finalFieldName))) {
                            throw new JSON5SerializerException("Invalid @ObtainTypeValue method of " + currentType.getName() + "." + method.getName() + ".field with annotated @JSON5Value '" + fieldName + "' not found in " + currentType.getName());
                        }
                    ...
                    for(String setterMethodName : setterMethodNames) {
                        ...
                        if(allMethods.stream().filter(methodObj -> methodObj.getAnnotation(JSON5ValueSetter.class) != null).noneMatch(methodObj -> methodObj.getName().equals(finalMethodName))) {
                            throw new JSON5SerializerException("Invalid @ObtainTypeValue method of " + currentType.getName() + "." + method.getName() + ". Method '" + methodName + "' not found in " + currentType.getName());
                        }
    ...
    private static Class<?> verifyMethod(Set<String> genericTypeNames, Class<?> currentType, Method method) {
        ...
        else if(!Types.isSingleType(Types.of(returnType)) && (returnType == void.class || returnType == Void.class || returnType == null || (!genericType && returnType.getAnnotation(JSON5Type.class) == null))) {
            throw new JSON5SerializerException("Invalid @ObtainTypeValue method of " + currentType.getName() + "." + method.getName() + ".  Return type must be a class annotated with @JSON5Type");
        }
        int parameterCount = method.getParameterCount();
        Class<?>[] parameterTypes = method.getParameterTypes();
        if(parameterCount > 0) {
            Class<?> parameterType = parameterTypes[0];
            if(!parameterType.isAssignableFrom(JSON5Object.class)) {
                throw new JSON5SerializerException("Invalid @ObtainTypeValue method of " + currentType.getName() + "." + method.getName() + ". Parameter type only can be JSON5Object");
            }
        }
        if(parameterCount > 1) {
        ...
        if(parameterCount > 2) {
            throw new JSON5SerializerException("Invalid @ObtainTypeValue method of " + currentType.getName() + "." + method.getName() + ".  Parameter count must be zero or one of JSON5Element or JSON5Object or JSON5Array");
        }
        return returnType;
    }
    ...
    Object obtain(Object parents, JSON5Object item, JSON5Object all) {
        try {
            if(parameters == null || parameters.length == 0) {
                ...
                throw new JSON5SerializerException("Invalid @ObtainTypeValue method " + method.getName() + " of " + method.getDeclaringClass().getName() + ".  Parameter count must be zero or one of JSON5Element or JSON5Object or JSON5Array");
            }
        } catch (Exception e) {
            if(ignoreError) {
                return null;
            }
            throw new JSON5SerializerException("Failed to invoke @ObtainTypeValue method " + method.getName() + " of " + method.getDeclaringClass().getName(), e);
        }
    }
