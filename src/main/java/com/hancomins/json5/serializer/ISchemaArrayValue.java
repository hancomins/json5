...
interface ISchemaArrayValue extends ISchemaValue {
    ...
    static List<CollectionItems> getGenericType(Type type, String path) {
        if(type == null) {
            throw new JSON5ObjectException("Unknown collection or RAW type. Collections must use <generic> types. (path: " + path + ")");
        }
        ...
        if (genericFieldType instanceof ParameterizedType) {
            ...
            throw new JSON5ObjectException("Collections must use <generic> types. (path: " + path + ")");
        }
    }
        ...
        if(fieldArgTypes[0] instanceof Class<?>) {
            ...
            if(!(rawType instanceof Class<?>) || !Collection.class.isAssignableFrom((Class<?>)rawType)) {
                ...
                if(Map.class.isAssignableFrom((Class<?>) rawType)) {
                    throw new JSON5ObjectException("java.util.Map type cannot be directly used as an element of Collection. Please create a class that wraps your Map and use it as an element of the Collection. (path: " + path + ")");
                }
            ...
            throw new JSON5ObjectException("Invalid collection or RAW type. Collections must use <generic> types. (path: " + path + ")");
        }
    }
